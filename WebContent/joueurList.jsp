...
<body>
    ...
    <table class="schedule-table table">
        ...
        <tbody>
            <%-- Retrieve the list of joueurs from the request attribute --%>
            <% List<Joueur> joueurs = (List<Joueur>) request.getAttribute("listOfJoueurs"); %>
            <% if (joueurs != null && !joueurs.isEmpty()) { %>
                <% for (Joueur joueur : joueurs) { %>
                    ...
                    <img src="<%= joueur.getImage_url() %>" class="player-image">
                        ...
                        <td class="center"><%= joueur.getDateNaissance() %></td>
                        <td class="center"><%= joueur.getPays() %></td>
                        <td class="center"><%= joueur.getSalaire() %></td>
                        <%-- Retrieve the equipe object using the equipeId --%>
                        <%
                            Equipe equipe = null;
                            for (Equipe e : listOfEquipes) {
                                if (e.getEquipeId() == joueur.getEquipeId()) {
                                    equipe = e;
                                    break;
                                }
                            }
                        %>
                        <td class="center"><%= (equipe != null) ? equipe.getNom() : "" %></td>
                        <td class="center" style="display: flex; align-items: center; justify-content: center;">
                            <a class="btn custom-btn smoothscroll" href="JoueurEditionController?joueurId=<%= joueur.getJoueurId() %>&mode=Modifier">Modifier</a>
                            <a class="btn custom-btn smoothscroll" href="JoueurEditionController?joueurId=<%= joueur.getJoueurId() %>&mode=Suppression" onclick="return confirm('Voulez-vous vraiment supprimer cette équipe ?')">Supprimer</a>
                        </td>
                    </tr>
                <% } %>
            <% } else { %>
  ...
  <style>
    ...
    .image-column {
     ...
     .center {
      ...
      <a class="link-fx-1 color-contrast-higher mt-4" href="<%= request.getContextPath() %>/formJoueur.jsp">
